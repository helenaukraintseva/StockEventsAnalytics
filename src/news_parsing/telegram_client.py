...
def parse_telegram_news(channel_title: str, days_back: int):
    """
    Парсит Telegram-канал и возвращает список сообщений за последние дни.

    :param channel_title: username канала без @
    :param days_back: количество дней назад для фильтрации
    :return: список словарей с текстом, датой, временем и URL
    """
    try:
    ...
    with client:
        while True:
            messages = client.get_messages(
                f"@{channel_title}", limit=limit, offset_id=offset_id
            )
            if not messages:
                break
...
if __name__ == "__main__":
    news = parse_telegram_news("if_market_news", days_back=3)
    for item in news:
