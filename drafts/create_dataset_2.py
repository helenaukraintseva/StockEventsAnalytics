...
import pandas as pd
import numpy as np
from settings import tokens_group_0_1, tokens_group_1_10, tokens_group_10_100


class CSVBatchBuilder:
    def __init__(self, tokens: list, limit: str = "0a1", interval="1m",
                 time_col="open_time", close_col="close", features=None):
        """
    ...
    def build_regression_batches(self, window_size=10, predict_step=1):
        ...
        all_y = []
        part = 1
        out_csv = f"datasets/re_{self.limit}_i{self.interval}_w{window_size}_s{predict_step}_p{part}.csv"
        ...
        for file_path in self.csv_files:
            ...
            for start_i in range(0, limit, 5):
    ...
    def build_classification_batches(self, window_size=10, predict_step=1, threshold=0.0):
        ...
        all_y = []
        part = 1
        out_csv = f"datasets/cl_{self.limit}_i{self.interval}_w{window_size}_s{predict_step}_p{part}.csv"
        if self._test_exist_file(out_csv):
            print(f"Файл {out_csv} уже существует.")
            return
...
def train_cycle(tokens, limit: str = "0a1"):
    ...
    for interval in intervals:
        ...
        builder = CSVBatchBuilder(interval=interval, time_col="time",
                                  close_col="close", tokens=tokens,
                                  limit=limit)
...
if __name__ == "__main__":
    tokens = [f"crypto_data/{elem}" for elem in tokens_group_0_1]
    train_cycle(tokens=tokens, limit="0a1")
    tokens = [f"crypto_data/{elem}" for elem in tokens_group_1_10]
    train_cycle(tokens=tokens, limit="1a10")
    tokens = [f"crypto_data/{elem}" for elem in tokens_group_10_100]
    train_cycle(tokens=tokens, limit="10a100")
