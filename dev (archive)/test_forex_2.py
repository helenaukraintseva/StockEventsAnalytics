        ...
        while not self.is_done():
            ...
            for model in models:
                ...
                if self.pending_signals[model_name] is None and action in ["buy", "sell"]:
                    ...
                    if action == "buy":
                        best_row = lookahead_window.loc[lookahead_window[self.value_column].idxmax()]
                    else:
                        best_row = lookahead_window.loc[lookahead_window[self.value_column].idxmin()]

                    # Исполняем сделку на лучшей цене
                    best_price = best_row[self.value_column]
                    best_price = best_price.values[0]
